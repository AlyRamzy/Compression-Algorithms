...
public:
  ...
  double binary_to_decimal(const std::bitset<32>& fraction);
  ...
  std::bitset<32> decimal_to_binary(double fraction);
  ...
  // message.
  int encode(const std:: vector<unsigned char>& msg,
  ...
  void decode(const std::vector<std::bitset<32> >& encoded_msg,
              int num_symbols_per_block,
